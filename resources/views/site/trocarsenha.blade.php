...
<body>
    <div class="login-container">
        ...
        <div class="login">
            <form action="{{ url('site/trocarsenha') }}" method="post">
                @csrf
                <h2>Trocar Senha</h2>
                ...
                <input id="token" name="token" type="hidden" class="{{ $errors->has('token') ? 'input-error' : '' }}" value="{{ $token }}">
                <input id="email" name="email" type="hidden" class="{{ $errors->has('token') ? 'input-error' : '' }}" value="{{ Request::get('email') }}">
